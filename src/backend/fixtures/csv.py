...
async def read_csv(filepath: str) -> list[dict]:
    ...
    data = []
    async with aiofiles.open(filepath, mode='r', encoding='utf-8-sig', newline='') as csvfile:
...
async def write_csv(data: list[tuple], filepath: str) -> None:
    ...
    async with aiofiles.open(filepath, mode='w', encoding='utf-8-sig', newline='') as csvfile:
