...
async def _update_menu_item_form_builder(
    ...
    menu_item_data['created_at'], menu_item_data['updated_at'] = [datetime.strptime(el, '%Y-%m-%dT%H:%M:%S.%f').strftime(s.DATETIME_FORMAT) for el in (menu_item_data['created_at'], menu_item_data['updated_at'])]

    ui.label(f'Запись создана {menu_item_data['created_at']} пользователем {menu_item_data['created_by']['username']}, изменена {menu_item_data['updated_at']} пользователем {menu_item_data['updated_by']['username']}').classes('text-subtitle2')
...
async def _save_menu_item_button_handler(
    ...
    try:
        ...
        if action == 'update':
            ...
            result = await menu_api_client.update_menu_item(id, new_menu_item.model_dump())
...
async def _delete_menu_item_button_handler(
    ...
    await menu_api_client.delete_menu_item(id)
...
async def menu_page(
    ...
    ui.item_label(f'Всего записей: {menu_page.total}').props('header').classes('text-h6')
    ...
    for menu_item in menu_page.items:
        with ui.card().classes('w-full'):
            with ui.grid(columns=2):
                ...
                ui.button('ИЗМЕНИТЬ',
                        on_click=lambda m_id=menu_item.id: ui.navigate.to(f'{m_id}')).visible = permission
                ui.button('УДАЛИТЬ',
                        on_click=partial(lambda m_id=menu_item.id: _delete_menu_item_button_handler(m_id))).visible = permission

    # Выводим кнопки пагинации
    if menu_page.pages > 1:
        chosen_page = ui.pagination(1, menu_page.pages,
...
async def menu_item_create_page(
    ...
    new_menu_item_data = {
        'button_text': None, 'answer': None, 'created_by_id': current_user.id, 'updated_by_id': current_user.id}

    # Выводим заголовок
    ui.item_label('Новый элемент справочника').props('header').classes('text-bold text-h4')
...
async def menu_item_update_page(
    ...
    menu_item_data: dict | None = await menu_api_client.get_menu_item(id)
    ...
    if menu_item_data is None:
        ...
        # Выводим заголовок
        ui.item_label('Изменение элемента справочника').props('header').classes('text-bold text-h4')
