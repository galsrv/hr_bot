...
class MessagesApiClient(BaseApiClient):
    ...
    async def get_chat(self, id: int) -> EmployeeChatSchema:
        '''Получить чат от бэкенда.'''
        url = f'{settings.API_URL}/{self.MODULE_URL}/employees/{id}/chat?page=1&size=100'
        chat = await self.get(url)
        chat = EmployeeChatSchema.model_validate(chat)
        return chat

    async def mark_chat_as_read(self, id: int) -> dict:
        '''Пометить сообщения в чате прочитанными.'''
        url = f'{settings.API_URL}/{self.MODULE_URL}/employees/{id}/chat/mark_as_read'
    ...
    async def ban_unban_employee(self, id: int, data_input: dict) -> dict:
        '''Пометить сообщения в чате прочитанными.'''
        url = f'{settings.API_URL}/{self.MODULE_URL}/employees/{id}'
